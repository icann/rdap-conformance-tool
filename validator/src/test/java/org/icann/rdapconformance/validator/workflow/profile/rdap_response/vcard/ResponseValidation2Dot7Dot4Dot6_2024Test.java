...
public class ResponseValidation2Dot7Dot4Dot6_2024Test extends ProfileJsonValidationTestBase {
    ...
    static final String pathLangBadPointer =
            "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"removal\",\"name\":{\"type\":\"Registrant Postal Code\"},\"postPath\":\"$test\",\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}";
    static final String postPathNotExistingPointer =
            "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"removal\",\"name\":{\"type\":\"Registrant Postal Code\"},\"postPath\":\"$.status[*]\",\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}";
    static final String methodPointer =
            "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"test2\",\"name\":{\"type\":\"Registrant Postal Code\"},\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}";
    ...
    public ProfileValidation getProfileValidation() {
        return new ResponseValidation2Dot7Dot4Dot6_2024(queryContext);
    }
    ...
    public void ResponseValidation2Dot7Dot4Dot6_2024_63602_By_PathLang_NotValid() {
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0).getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");
    ...
    @Test
    public void testPostalCodeNotEmptyAndRedactionExists_ShouldTrigger63605() {
        // Set postal code to a non-empty value
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0)
                .getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        postalCodeValue.put(5, "G1V 2M2"); // Non-empty postal code

        // Ensure redacted array contains a Registrant Postal Code redaction object
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");

        // Should trigger -63605
        validate(-63605,
                "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"removal\",\"name\":{\"type\":\"Registrant Postal Code\"},\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}",
                "a redaction of type Registrant Postal Code was found but the postal code was not redacted.");
    }

    @Test
    public void testPostalCodeNotEmptyAndNoRedaction_ShouldNotTrigger63605() {
        // Set postal code to a non-empty value
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0)
                .getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        postalCodeValue.put(5, "G1V 2M2"); // Non-empty postal code

        // Remove all Registrant Postal Code redaction objects
        JSONArray redactedArray = jsonObject.getJSONArray("redacted");
        for (int i = 0; i < redactedArray.length(); i++) {
            JSONObject redactedObject = redactedArray.getJSONObject(i);
            JSONObject name = redactedObject.getJSONObject("name");
            if (name.has("type") && name.getString("type").equals("Registrant Postal Code")) {
                name.put("type", "Other Redaction");
            }
        }

        // Should NOT trigger -63605 (no validation errors expected)
        validate();
    }

    @Test
    public void testRedactedObjectMissingNameProperty_ShouldNotFail() {
        // Add a redacted object with no 'name' property
        ...
        // Postal code is empty to trigger redaction validation
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0)
                .getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        postalCodeValue.put(5, "");
        // Should not throw, should pass validation
        validate();
    }

    @Test
    public void testRedactedObjectMissingPostPathWithPathLang_ShouldNotFail() {
        // Redacted object with pathLang but missing postPath
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");
    ...
    @Test
    public void testRedactedObjectMissingMethodProperty_ShouldNotFail() {
        // Redacted object with all properties except method
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");
        redactedObject.put("pathLang", "jsonpath");
        redactedObject.put("postPath", "$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][5]");
    ...
    @Test
    public void testRedactedObjectIsNotJSONObject_ShouldNotFail() {
        // Add a redacted object that is a string
        jsonObject.getJSONArray("redacted").put("not-a-json-object");
        // Postal code is empty to trigger redaction validation
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0)
                .getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        postalCodeValue.put(5, "");
    ...
    @Test
    public void testRedactedObjectMissingPathLangProperty_ShouldNotFail() {
        // Redacted object missing pathLang
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");
        redactedObject.remove("pathLang");
        redactedObject.put("postPath", "$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][5]");
    ...
    @Test
    public void testRedactedObjectMissingPostPathProperty_ShouldNotFail() {
        // Redacted object missing postPath
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");
        redactedObject.put("pathLang", "jsonpath");
        redactedObject.remove("postPath");
    ...
    @Test
    public void testRedactedObjectMissingMethodPropertyWithAllOtherValidations_ShouldNotFail() {
        // Redacted object missing method, all other validations pass
        JSONObject redactedObject = jsonObject.getJSONArray("redacted").getJSONObject(0);
        redactedObject.getJSONObject("name").put("type", "Registrant Postal Code");
        redactedObject.put("pathLang", "jsonpath");
        redactedObject.put("postPath", "$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][5]");
    ...
    @Test
    public void testRedactedObjectWithNameDescriptionOnly_ShouldNotFail() {
        // Add a redacted object with only name.description
        JSONObject redactedObject = new JSONObject();
        redactedObject.put("name", new JSONObject().put("description", "Administrative Contact"));
        redactedObject.put("reason", new JSONObject().put("description", "Server policy"));
        redactedObject.put("method", "emptyValue");
        jsonObject.getJSONArray("redacted").put(redactedObject);
        // Postal code is empty to trigger redaction validation
    ...
    @Test
    public void testRedactedNameTypeMatches_ShouldExtractRedactedPostalCode() {
        // Add a redacted object with name.type matching "Registrant Postal Code"
        JSONObject redactedObject = new JSONObject();
        redactedObject.put("name", new JSONObject().put("type", "Registrant Postal Code"));
        redactedObject.put("reason", new JSONObject().put("description", "Server policy"));
        redactedObject.put("method", "emptyValue");
        jsonObject.getJSONArray("redacted").put(redactedObject);
        // Postal code is empty to trigger redaction validation
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0)
            .getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        postalCodeValue.put(5, "");
        // Should pass validation, redactedPostalCode is found
        validate();
    }

    @Test
    public void testRedactedNameTypeMatchesWithSpacesAndCase_ShouldExtractRedactedPostalCode() {
        // Add a redacted object with name.type matching "  registrant postal code  " (spaces, case-insensitive)
        JSONObject redactedObject = new JSONObject();
        redactedObject.put("name", new JSONObject().put("type", "  registrant postal code  "));
        redactedObject.put("reason", new JSONObject().put("description", "Server policy"));
        redactedObject.put("method", "emptyValue");
    ...
    @Test
    public void testRedactedNameTypeDoesNotMatch_ShouldNotExtractRedactedPostalCode() {
        ...
        JSONArray postalCodeValue = jsonObject.getJSONArray("entities").getJSONObject(0)
            .getJSONArray("vcardArray").getJSONArray(1).getJSONArray(3).getJSONArray(3);
        postalCodeValue.put(5, "");
        // Should trigger -63601 (no Registrant Postal Code redaction found)
        validate(-63601, "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"removal\",\"name\":{\"type\":\"Tech Phone\"},\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}, #/redacted/2:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Other Redaction\"}}", "a redaction of type Registrant Postal Code is required.");
    }

    @Test
    public void testRedactedNameTypeIsNonString_ShouldNotExtractRedactedPostalCode() {
        ...
        postalCodeValue.put(5, "");
        // Should trigger -63601 (no Registrant Postal Code redaction found)
        validate(-63601, "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"removal\",\"name\":{\"type\":\"Tech Phone\"},\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}, #/redacted/2:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":123}}", "a redaction of type Registrant Postal Code is required.");
    }
    ...
    public void testRedactedNameTypeMissing_ShouldCatchExceptionAndContinue() {
        ...
        postalCodeValue.put(5, "");
        // Should trigger -63601 (no Registrant Postal Code redaction found)
        validate(-63601, "#/redacted/0:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"removal\",\"name\":{\"type\":\"Tech Phone\"},\"prePath\":\"$.entities[?(@.roles[0]=='technical')].vcardArray[1][?(@[1].type=='voice')]\"}, #/redacted/1:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{\"type\":\"Registrant Street\"},\"postPath\":\"$.entities[?(@.roles[0]=='registrant')].vcardArray[1][?(@[0]=='adr')][3][:3]\",\"pathLang\":\"jsonpath\"}, #/redacted/2:{\"reason\":{\"description\":\"Server policy\"},\"method\":\"emptyValue\",\"name\":{}}", "a redaction of type Registrant Postal Code is required.");
    }
}

...
public class ResponseValidationRFC5731Test extends ResponseDomainValidationTestBase {
  ...
  public static Object[][] invalidStatus() {
    return new Object[][]{{Set.of("active", "client delete prohibited")},
            ...
            // New denylist coverage — each prohibited status combined with "active"
            {Set.of("active", "client hold")},
            {Set.of("active", "client renew prohibited")},
            {Set.of("active", "client transfer prohibited")},
            {Set.of("active", "client update prohibited")},
            {Set.of("active", "server renew prohibited")},
            {Set.of("active", "server transfer prohibited")},
            {Set.of("active", "server update prohibited")},
            {Set.of("active", "pending create")},
            {Set.of("active", "pending renew")},
            {Set.of("active", "pending update")},
            {Set.of("active", "pending transfer")},
  ...
  public void testDoLaunch() {
    ...
    assertThat(new ResponseValidationRFC5731(helpContext).doLaunch()).isFalse();
    ...
    assertThat(new ResponseValidationRFC5731(nameserversContext).doLaunch()).isFalse();
    ...
    assertThat(new ResponseValidationRFC5731(nameserverContext).doLaunch()).isFalse();
    ...
    assertThat(new ResponseValidationRFC5731(entityContext).doLaunch()).isFalse();
    ...
    domainContext.setRdapResponseData(queryContext.getRdapResponseData());
    assertThat(new ResponseValidationRFC5731(domainContext).doLaunch()).isTrue();
  }
  ...
  public static Object[][] validActiveStatus() {
    return new Object[][]{{Set.of("active")},
            ...
            // Now valid for 46900: statuses NOT in the denylist may combine with "active"
            // (redemption period / pending restore are handled by other validations, not 46900)

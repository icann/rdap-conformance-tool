...
public class RDAPHttpQueryTest extends HttpTestingUtils {
    ...
    public static Object[][] serverFault() {
        return new Object[][]{{Fault.EMPTY_RESPONSE}, {Fault.RANDOM_DATA_THEN_CLOSE}, {Fault.MALFORMED_RESPONSE_CHUNK},
    ...
    public static Object[][] serverTlsErrors() {
        ...
        return new Object[][]{{"https://expired.badssl.com", ConnectionStatus.EXPIRED_CERTIFICATE},
            ...
            // we don't test for untrusted roots, we just continue on.
    ...
    public void setUp(Method method) {
        super.setUp();
        WireMockConfiguration wmConfig = wireMockConfig().dynamicHttpsPort().bindAddress(WIREMOCK_HOST);
    ...
    public void test_WithHttps_LocalTrustStore() {
        givenUri("https");
        stubFor(get(urlEqualTo(REQUEST_PATH)).withPort(wireMockServer.httpsPort())
                                             .withScheme("https")
    ...
    public void test_ServerFaultWithHttps_LocalTrustStore(Fault fault) {
        givenUri("https");
        stubFor(get(urlEqualTo(REQUEST_PATH)).withScheme("https")
                                             .willReturn(aResponse().withHeader("Content-Type",
                                                 "application/rdap+JSON;encoding=UTF-8").withFault(fault)));
        ...
        stubFor(get(urlEqualTo(REQUEST_PATH)).withScheme(HTTP)
                                             .willReturn(aResponse().withFixedDelay(2000)
                                                                    .withHeader("Content-Type",
                                                                        "application/rdap+JSON;encoding=UTF-8")
...
//    rdapHttpQuery.setResults(results);
        ...
        givenUri(HTTP, path1);
        stubFor(get(urlEqualTo(path1)).withScheme(HTTP).willReturn(temporaryRedirect(path2)));
        stubFor(get(urlEqualTo(path2)).withScheme(HTTP).willReturn(temporaryRedirect(path3)));
        stubFor(get(urlEqualTo(path3)).withScheme(HTTP)
                                      .willReturn(
                                          aResponse().withHeader("Content-Type", "application/rdap+JSON;encoding=UTF-8")
                                                     .withBody(RDAP_RESPONSE)));
    ...
    public void test_ServerRedirectMoreThanRetries_ReturnsErrorStatus16() throws Exception {
        ...
        try (MockedStatic<RDAPHttpRequest> mockedStatic = mockStatic(RDAPHttpRequest.class)) {
            ...
            when(response1.headers()).thenReturn(
                HttpHeaders.of(Map.of(LOCATION, List.of(uri2.toString())), (k, v) -> true));
            ...
            when(response2.headers()).thenReturn(
                HttpHeaders.of(Map.of(LOCATION, List.of(uri3.toString())), (k, v) -> true));
            ...
            when(response3.headers()).thenReturn(
    ...
    public void testIsBlindlyCopyingParams_NotCopied() {
        ...
        assertThat(result).isFalse();
        assertThat(results.getAll()).doesNotContain(RDAPValidationResult.builder()
                                                                        .code(-13004)
                                                                        .value("<location header value>")
                                                                        .message(
                                                                            "Response redirect contained query parameters copied from the request.")
    ...
    public void test_RedirectWithoutLocationHeader_BreaksLoop() {
        ...
        givenUri(HTTP, path1);
        stubFor(get(urlEqualTo(path1)).withScheme(HTTP)
                                      .willReturn(
    ...
    public void test_WithJsonArray() {
        ...
        givenUri(HTTP, path);
        stubFor(get(urlEqualTo(path)).withScheme(HTTP)
                                     .willReturn(
                                         aResponse().withHeader("Content-Type", "application/rdap+JSON;encoding=UTF-8")
                                                    .withBody(response)));
    ...
    public void testVerifyIfObjectClassPropExits_MissingObjectClassName_ReturnsFalse() {
        List<Map<String, Object>> propertyCollection = List.of(
            Map.of("entities", List.of(Map.of("objectClassName", "entity"))), Map.of("objectClassName", "nameserver"));

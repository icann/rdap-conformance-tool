...
public final class TigValidation3Dot2_2024 extends ProfileJsonValidation {
    ...
    private static final Pattern DOMAIN_QUERY_PATTERN_FLEXIBLE = Pattern.compile("^https?://[^/]+/.*domain/.+$");
        ...
        if (links != null) {
            for (Object link : links) {
                ...
                if (l.optString(REL_FIELD).equals(REL_RELATED) && l.has(HREF_FIELD)) {
                    ...
                    if (href.isEmpty() || !DOMAIN_QUERY_PATTERN_FLEXIBLE.matcher(href).matches()) {
                        RDAPValidationResult.Builder builder = RDAPValidationResult.builder()
                            .code(-23202)
                            .value(l.toString())

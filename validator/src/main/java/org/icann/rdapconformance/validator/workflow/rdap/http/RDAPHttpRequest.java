...
public class RDAPHttpRequest {
    ...
    public static ConnectionStatus handleRequestException(Exception e, boolean recordError) {
        // Delegate to session-aware version with default session
        String defaultSessionId = org.icann.rdapconformance.validator.session.SessionContext.validateSessionId(null);
        ConnectionTracker tracker = ConnectionTracker.getInstance(defaultSessionId);
        return handleRequestException(e, recordError, tracker);
    }

...
### Web URI validation [webUriValidation]
...
### Domain label case folding validation [domainCaseFoldingValidation]
...
### RDAP Conformance validation [stdRdapConformanceValidation]
...
3. Each of the JSON string values in the JSON array, with the exception of "rdap_level_0",
    shall be included as an Extension Identifier in **RDAPExtensions**.
    {
    "code": -10502 ,
    "value": "<JSON string>",
...
4. The JSON string value "rdap_level_0" is not included in the _RDAP Conformance_ data
    structure.
    {
    "code": -10503 ,
...
### Links validation [stdRdapLinksValidation]
...
```
2.2. The JSON name/value pairs of _rel_ , _href_ , _hreflang_ , _title_ , _media_ and _type_ shall appear
only once.
...
```
2.3. If the JSON name _media_ exists, the allowed values are: screen, tty, tv, projection,
handheld, print, braille, embossed, speech, and all.

```
{
"code": - 10603 ,
"value": "<name/value pair>",
"message": "The value for the JSON name media is not of: screen, tty, tv,
projection, handheld, print, braille, embossed, speech, or all."
}
```
2.4. If the JSON name _rel_ exists, the value shall be included as a "Relation Name" in
linkRelations.
...
```
2.5. If the JSON name _type_ exists, the value shall be included as a "Type Name/Subtype
Name" as registered in mediaTypes.
...
```
2.7. If the JSON name hreflang exists, the value shall be a JSON string data type or a
valid JSON array where every value is a JSON string data type.
```
{
"code": - 10607 ,
"value": "<name/value pair>",
...
```
2.8. If the JSON name _hreflang_ exists, every one of the _JSON string_ data values shall
conform to the Language-Tag syntax.
...
```
2.9. If the JSON name _value_ exists, the value shall pass the test Web URI validation
[webUriValidation] defined in this document.
...
```
2.11. For the JSON name href , the value shall pass the test Web URI validation
[webUriValidation] defined in this document.
```
{
"code": - 10611 ,
...
```
2.4. If the JSON name links exists, the value shall pass the test Links validation
...
```
2.6. If the JSON name type exists, the value shall be included in the RDAPJSONValues
with Type="notice and remark type".
```
{
"code": - 10706 ,
"value": "<JSON string>",
...
```
2.9. Every value of the JSON array of the _description_ data structure shall be a JSON
...
### Language Identifier Validation [stdRdapLanguageIdentifierValidation]
...
### Events Validation [stdRdapEventsValidation]
...
2. For every object of the JSON array, verify that the structure complies with:
2.1. The name of every name/value pair shall be any of: eventAction , eventActor ,
eventDate or links.
...
```
2.2. The JSON name/value pairs of _eventAction_ , _eventActor_ , _eventDate_ and _links_ shall
exist only once.
...
```
2.5. For the JSON name _eventAction_ , the value shall be included in the
...
```
2.8. For the JSON name eventDate , the value shall be syntactically valid time and date
...
```
2.11. If the JSON name _links_ exists, the value shall pass the test Links validation
[stdRdapLinksValidation] defined in this document.
...
### Status validation [stdRdapStatusValidation]
...
3. Each of the JSON string values in the JSON array shall be included in the
    **RDAPJSONValues** with Type="status".
    {
    "code": - 11002 ,
    "value": "<JSON string>",
...
### Port 43 WHOIS Server [stdRdapPort43WhoisServerValidation]

The following steps should be used to test that a port43 data structure is valid:

1. For the JSON name _port43_ , the value shall pass the test [IPv4Validation],
[IPv6Validation] or [DomainNameValidation] defined in this document.
...
### Public IDs validation [stdRdapPublicIdsValidation]
...
2. The _asEventActor_ data structure must be embedded within an entity object, and the
    entity object must be embedded within another object.
    {
    "code": - 11301 ,
    "value": "<asEventActor structure>",
    "message": "The asEventActor structure is not embedded within an entity
    object and the entity object is not embedded within another object."
    }
...
"code": - 11302 ,
"value": "<name/value pair>",
"message": "The name in the name/value pair is not of: eventAction and
eventDate."
}
```
...
```
3.5. For the JSON name eventAction , the value shall be included in the
RDAPJSONValues with Type="event action".
...
```
3.8. For the JSON name _eventDate_ , the value shall be syntactically valid time and date
...
### IP Addresses Validation [stdRdapIpAddressesValidation]
